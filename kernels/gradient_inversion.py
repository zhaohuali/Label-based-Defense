...
class BaseGradientInversion():
    ...
    def __init__(
        ...
        self.target_tv = args.target_tv
        self.init_time = args.init_time
    ...
    def run(self, lr_start_end:tuple, 
        ...
        try:
            stop = torch.tensor([0], device=x_pseudo.device)
            for epoch in range(1, epochs+1):
                ...
                with torch.no_grad():
                    ...
                    if self.min_grads_loss:
                        if simple_total_variation(self.best_x_pseudo) < self.target_tv:
                            stop[0] = 1
                    else:
                        if simple_total_variation(x_pseudo) < self.target_tv:
                            stop[0] = 1
        ...
        if self.min_grads_loss:
            end_x_pseudo, end_loss_track =  self.best_x_pseudo, self.best_loss_track
        else:
            end_x_pseudo, end_loss_track =  x_pseudo, self.loss_track

        self.grads_inversion(end_x_pseudo, y_pseudo, optimizer, state='compute_layer_loss')()

        return end_x_pseudo, end_loss_track
            
    def early_stop(self, stop):
        dist.all_reduce(stop.data, op=dist.ReduceOp.SUM)
    ...
    def grads_inversion(self, x_pseudo, y_pseudo, optimizer=None, state='update'):
        ...
        def closure():
            ...
            if (state == 'update'):
                create_graph = True
                    ...
                    or state == 'compute_layer_loss'):
                create_graph = False
            else:
                raise ValueError(f'state:{state}')

            gradent_pseudo = torch.autograd.grad(loss_pseudo, 
                                                 filter(
            ...
            gradient_loss = self.compute_loss(x_pseudo, gradent_pseudo, state=state)
    ...
    def compute_loss(
        ...
        if state == 'compute_layer_loss':
            self.compute_layers_grads_loss(gradent_pseudo)
            return None
    ...
    # TODO: Add layer-level loss printing
    def compute_layers_grads_loss(self, gradent_pseudo):
    ...
    def compute_regularization(self, x_pseudo):
        ...
        x_norm_loss = torch.norm(x_pseudo.view(x_pseudo.size(0), -1), p=2, dim=-1).mean()
    ...
    def update_best_x_pseudo(self, x_pseudo):
        ...
        with torch.no_grad():
            if self.loss_track['gradient loss'] < self.best_loss_track['gradient loss']:
                self.best_x_pseudo = x_pseudo.detach().clone()
                self.best_x_pseudo.data = self.boxing_input(self.best_x_pseudo)

                self.best_loss_track['gradient loss'] = self.loss_track['gradient loss']
                self.best_loss_track['total loss'] = self.loss_track['total loss']
    ...
    def display_results(self, epoch, epochs, flag=''):
        ...
        print(f'>> [{epoch}/{epochs}] clock: {(time.time() - self.init_time):.2f}')
