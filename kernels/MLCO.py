...
class MLCOGradientInversion(BaseGradientInversion):
    ...
    def __init__(self, target_gradient, model, dm, ds, args) -> None:
        ...
        self.total_T_in = math.ceil(args.total_T_in_rate * args.epochs/args.T_in)
        self.T_in = args.T_in
        ...
        for i in range(self.total_T_in):
            ...
            for i_seed in range(n_seed):
                ...
                x_pseudo_list[i_seed], loss_track = self.run(
                    ...
                    flag=f'MLCO-in({i+1}/{self.total_T_in})-seed({i_seed+1}/{n_seed})')
    ...
    def get_best_pseudo(self, loss_list, x_pseudo_list, y_pseudo):
        ...
        min_loss = self.grads_inversion(x_pseudo, y_pseudo, state='get_gradient_loss')().item()
        ...
        if self.distributed:
            rank = dist.get_rank()
            self.distributed_wait(x_pseudo, y_pseudo, batch_size, n_seed, min_loss_idx, min_loss, wait='init')
        ...
        for i_samples in range(batch_size):
            ...
            for i_seed in range(n_seed):
                ...
                loss = self.grads_inversion(x_test, y_pseudo, state='get_gradient_loss')().item()
                if loss < min_loss:
                    min_loss = loss
                    best_seed = i_seed

            x_pseudo[i_samples] = x_pseudo_list[best_seed][i_samples].detach().clone()
            print(f'i_imgs: {i_samples + rank*batch_size} best_seed:{best_seed} loss: {min_loss}')
        
        # min_loss = self.grads_inversion(x_pseudo, y_pseudo, state='get_gradient_loss')().item()
        # print(f'[rank] {rank} min loss: {min_loss}')

        if self.distributed:
            self.distributed_wait(x_pseudo, y_pseudo, batch_size, n_seed, min_loss_idx, min_loss, wait='end')
    ...
    def distributed_wait(self, x_pseudo, y_pseudo, batch_size, n_seed, min_loss_idx, min_loss, wait='init'):
        ...
        for i_rank in range(n_wait_rank):
            for i_samples in range(batch_size):
                best_seed = min_loss_idx
                for i_seed in range(n_seed):
                    loss = self.grads_inversion(x_pseudo, y_pseudo, state='get_gradient_loss')().item()
                    if loss < min_loss:
                        min_loss = loss
                        best_seed = i_seed
                if rank == 0:
                    print(f'i_imgs: {i_samples + (i_rank+s_rank) * batch_size} best_seed:{best_seed} loss: {min_loss}')
