...
def get_gir(model, metric_dict, model_eval):
    ...
    column_name = ['T:IDX', 'T:Label', 'T:confi', 'Max:Label', 'Max:confi', 'G Norm', 'G L2', 'G Sim', 'G Sign Rate(%)', 'G info', 'GIR(%)']
    ...
    for i in range(bs):
        ...
        sim_diff, n_sign = compute_sim_loss_and_sign_rate(single_grads, total_grads)

        gir['G Sim'] = sim_diff
        gir['G Sign Rate(%)'] = int(n_sign* 100 /total_elem)
        gir['G Sign Rate(%)'] = 0

        single_grads_norm = torch.stack([g.norm() for g in single_grads]).mean()
...
# y = torch.randint(1000, size=(bs,))
# table = get_gir(model, x, y, model_eval=True)
# print(table)