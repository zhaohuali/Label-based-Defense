...
def save_imgs(x, y, dm, ds, distributed, input_size, rank, dir_path, clock, str_=''):
    ...
    if not distributed or rank == 0:
        ...
        imgs = to_img(x, dm, ds)
        plt_imgs(imgs, y, n_channels, title=f'F')
...
def save_results_with_metric(x, y, dm, ds, metric_dict, args, str_=''):
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed
        ...
        if args.metric:
            ...
            plt_imgs(true_imgs, y_true, n_channels, title=f'T')
...
def compute_metric(x_recon, x_true, dm, ds, args):
    ...
    if args.one_to_one_similarity:
        ...
        metrics_batch = metric.batch_similarity(true_imgs, 
                                                imgs, 
                                                T_to_R_map=torch.arange(0, min(len(true_imgs),len(imgs))))
    else:    
        metrics_batch = metric.min_similarity(true_imgs,
                                            imgs,)
...
class ImageSimilarity():
    ...
    def batch_similarity(self, target_batch, 
                               recon_batch,
                               T_to_R_map):
    ...
    def perceptual_metric(self, target, recon):
        """LPIPS论文代码:https://github.com/richzhang/PerceptualSimilarity"""
        if target.size()[-1] < 32:
            # 该测量要求输入像素点大于32
    ...
    def calc_ssim(self, target, recon):
        ...
        ssim_score = ssim(target_np, recon_np, data_range=255, multichannel=multichannel)
